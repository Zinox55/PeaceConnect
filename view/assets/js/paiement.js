...
function afficherResume() {
    ...
    panierData.articles.forEach(article => {
        ...
        html += `
            <div style="display: flex; align-items: center; gap: 15px; padding: 15px; border-bottom: 1px solid #eee;">
                <img src="${article.image || '../assets/img/default.jpg'}" alt="${article.nom}" 
                ...
                <div style="text-align: right;">
                    <p style="margin: 0; font-weight: 600; color: #5F9E7F;">${sousTotal.toFixed(2)} €</p>
                    <p style="margin: 0; color: #999; font-size: 0.85rem;">${article.prix.toFixed(2)} € / unité</p>
                </div>
            </div>
        `;
    });
    ...
    html += `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 20px; background: #f8f9fa; border-radius: 8px;">
            ...
            <h2 style="margin: 0; color: #5F9E7F; font-size: 2rem;">${panierData.total.toFixed(2)} €</h2>
        </div>
    `;
...
document.querySelectorAll('.payment-option').forEach(option => {
    option.addEventListener('click', function() {
        ...
        if (methodePaiementSelectionnee === 'paypal') {
            ...
            if (methodePaiementSelectionnee === 'card') {
                btnPayer.innerHTML = '<i class="fas fa-lock"></i> Payer ' + panierData.total.toFixed(2) + ' €';
            } else if (methodePaiementSelectionnee === 'stripe') {
                btnPayer.innerHTML = '<i class="fas fa-lock"></i> Payer avec Stripe ' + panierData.total.toFixed(2) + ' €';
            } else if (methodePaiementSelectionnee === 'virement') {
