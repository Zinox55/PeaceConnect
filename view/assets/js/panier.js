...
function displayPanier(items, total) {
    ...
    items.forEach(item => {
        ...
        const stockWarning = stockRestant < 5 ? `<div style="color: #f39c12; font-size: 0.85rem; margin-top: 5px;"><i class="fas fa-exclamation-triangle"></i> Plus que ${item.stock} en stock</div>` : '';
                ...
                <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
                    ...
                    <div style="flex: 1; min-width: 200px;">
                        ...
                        <p style="margin: 5px 0 0 0; color: #6C757D; font-size: 0.9rem;">
                            <i class="fas fa-box"></i> Stock disponible: <strong>${item.stock}</strong>
                        </p>
                        ${stockWarning}
                    </div>
                    <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <input type="number" value="${item.quantite}" min="1" max="${item.stock}" class="quantite-input" data-id="${item.panier_id}" data-max="${item.stock}" style="width: 70px; padding: 8px; border: 2px solid #5F9E7F; border-radius: 6px; text-align: center; font-weight: 600; font-size: 1rem;" title="Max: ${item.stock}">
                        </div>
...
function attachPanierEvents() {
    ...
    document.querySelectorAll('.quantite-input').forEach(input => {
        input.addEventListener('change', function() {
            ...
            if (quantite < 1) {
                this.value = 1;
                showNotification('La quantité doit être au moins 1', false);
                return;
            }
            
            if (quantite > maxStock) {
                this.value = maxStock;
                showNotification(`Stock insuffisant (max: ${maxStock})`, false);
                return;
            }
            
            if (quantite > 0 && quantite <= maxStock) {
